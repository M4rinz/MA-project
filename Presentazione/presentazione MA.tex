...
\section{Matrici di operatori lineari}	
	...
	\begin{frame}{Intermezzo: cenni di \emph{Machine Learning}}
		\begin{block}{Remark 1:} 
			...
			Dunque $\dd{\mathcal{L}}=\left\langle\qty(\pdv{X_N}{\Theta})^{\!\!\top}\grad{L},\dd{\Theta}\right\rangle_F$.
			\smallskip
...
%		\end{itemize}
	...
	\begin{frame}
		...
		\begin{enumerate}
			...
			\item<4-> $\mathtt{M}'\,*\,(\dots)$ applica una matrice di \texttt{Operator}s a un vettore con dei \texttt{Float64}. Se $\mathtt{M}$ non è diagonale, va gestita la somma con uno zero {\smaller ($\equiv$ matrici nulle)} di dimensioni diverse!
		\end{enumerate}
