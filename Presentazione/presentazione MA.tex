...
\section{Matrici di operatori lineari}	
	...
	\begin{frame}{Derivata della \emph{loss} $\qquad 2/2$}{Ricavare il gradiente dalla matrice di adiacenza}
...
%	\end{frame}
	...
	\begin{frame}{Implementazione in Julia $\qquad 1/2$}{Overview del codice prodotto}
		\textbf{Disclaimer}: il codice era già fornito dagli autori dell'articolo originale. \`E stato solo leggermente rimaneggiato.
		\smallskip 
	...
	\begin{frame}
		...
		\begin{enumerate}
			...
			\item<4-> $\mathtt{M}'\,*\,(\dots)$ a sua volta applica una matrice di \texttt{Operator}s a un vettore con dei \texttt{Float64}
		\end{enumerate}
	...
	\begin{frame}{In conclusione\dots}{Commenti, e possibili estensioni}
		...
		Ad esempio {\smaller (cfr. Notebooks \algotext{1\_Valutazione.ipynb} e \algotext{2\_Inversione.ipynb})}:
		\begin{itemize}
			\item Nella composizione di operatori mancano controlli di ammissibilità, e potrebbe ben presto diventare inefficiente
			\item Valutare $\mathtt{M}'\ *\ (\mathtt{ImL}' \,\backslash\, \mathtt{g})$ in maniera alternativa presta il fianco a molti problemi 
			\begin{itemize}
				\item Inversione esplicita di una matrice di \texttt{Operators}
				\item Moltiplicazione di matrici di \texttt{Operators}
			\end{itemize} 
			\item Nella pratica, avere un ordinamento topologico del DAG è \emph{fondamentale}
		\end{itemize}
