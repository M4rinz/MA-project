...
\section{Grafi computazionali}
	...
	\begin{frame}{Grafi computazionali e differenziazione automatica}
		...
		\begin{table}[ht]
			...
			\midrule
				...
				Path product del cammino $i\to j$ & Derivata parziale di $j$ rispetto a $i$ \\
	...
	\begin{frame}[fragile]{Grafo computazionale di un MLP}
		\begin{algorithm}[H]
			\caption{MLP scalare, senza bias (inferenza)}
			\begin{algorithmic}[0]
				\Require \textcolor{orange}{Input pattern $x_0$},
				\textcolor{orange}{pesi iniziali $w_i$, $i\in[N]$},
				\textcolor{orange}{funzioni di attivazione $h_i$}
				\For{$i=1,\dots, N$}
				\State $x_i\gets h_i(w_ix_{i-1})$
				\State \textcolor{SkyBlue}{$\qty(\delta_i\gets h_i'(w_ix_{i-1}))$}
				\EndFor
				\Ensure \textcolor{orange}{rappresentazione dell'output layer $x_N$}
			\end{algorithmic}
		\end{algorithm}
	\end{frame}
