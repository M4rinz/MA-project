...
\section{Matrici di operatori lineari}	
	...
	\begin{frame}
		...
		\begin{columns}
			...
			\begin{column}{0.35\textwidth}
				\parbox[c][3ex][c]{\linewidth}{\centering\textbf{Rappresentazione densa}}
			\end{column}
		\end{columns}
		\hrule
	% 1a riga		
	...
	% 5a riga
		...
		Dove $\langle G,X\rangle_F\coloneqq\tr(G^{\top}X)=\sum_{i,j}G_{i,j}X_{i,j}$
	\end{frame}
	...
	\begin{frame}{MLP, caso generale $\qquad 1/3$}{Algoritmo e rappresentazione grafica}
		\begin{columns}
...
%				\]
		...
		%$h.$ denota l'applicazione pointwise di $h$
	\end{frame}
	...
	\begin{frame}{MLP, caso generale $\qquad 2/3$}{Differenziale della funzione di attivazione}
		...
		\begin{definizione}
			...
			\]
		\end{definizione}
		Ricordiamo che $\dd h.(A)=\qty(h.(A))'[\dd{A}]$ {\smaller (differenziale di Fréchet)}.
		\begin{align*}
			h.(A + \dd A) - h.(A) & = \qty(h(a_{i,j}+\dd{a}_{i,j}) - h(a_{i,j}))_{i,j} 
			= \qty(h'(a_{i,j})\dd{a_{i,j}})_{i,j}\\
			 & = h'.(A)\,.\!* \dd{A} \equiv \qty(h'.(A))_{\mathrm{H}}[\dd{A}]
		\end{align*}
		Dunque $\dd h.(A)$ è la funzione $\dd{A}\mapsto\qty(h'.(A))_{\mathrm{H}}[\dd{A}]$.
		\medskip 
		
		Il particolare per il MLP, $\dd{h.}(W_iX_i+b_ie^{\top})=(\Delta_i)_{\mathrm{H}}\,\dd\qty(W_iX_i+b_ie^{\top})$
	\end{frame}
	
	\begin{frame}
		{\hypertarget{frame:layer_differential}{MLP, caso generale $\qquad 3/3$}}
...
\section*{Appendice}
	...
	\begin{frame}
		...
		Per esplicitare il differenziale di $W_iX_{i-1}+b_ie^{\top}$ rispetto ai parametri, dobbiamo considerare tre casi: 
		\begin{enumerate}
		...
		In tutti e tre i casi abbiamo una funzione affine. Quindi {\smaller (in qualsiasi punto)} il differenziale è dato dalla parte lineare: 
		\begin{enumerate}
	...
	\begin{frame}{Dettagli sul calcolo del differenziale del layer $i$-esimo $\qquad 2/2$}
		...
		\]
		infatti per la regola della catena $\pdv{X_i}{\star}=\pdv{X_i}{(X_{i-1}W_i+B_i)}\pdv{(X_{i-1}W_i+B_i)}{\star}$; abbiamo visto che $\pdv{X_i}{(X_{i-1}W_i+B_i)}={\Delta_i}_{\mathrm{H}}$ e ad esempio  {\smaller ($\star = W_i$)} $\pdv{(X_{i-1}W_i+B_i)}{W_i}={X_{i-1}}_{\mathrm{R}}$
