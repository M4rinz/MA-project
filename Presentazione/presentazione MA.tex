...
\newcommand\algotext[1]{
	\colorbox{gray!70}{
		\!\!\includegraphics[height=0.7em]{Immagini/Julia3dots}
		\texttt{#1}\!\!
	}
}
...
\section{Matrici di operatori lineari}	
	...
	\begin{frame}{Intermezzo: cenni di \emph{Machine Learning}}
		...
		\begin{block}{Remark 2:} 
			...
			$\dd{X_N}=\pdv{X_N}{\Theta}\dd{\Theta}$. Dunque
		
			$
				\dd{\mathcal{L}}=\qty(\grad{L})^{\!\top\bullet}\pdv{X_N}{\Theta}\,\dd{\Theta}
				\xRightarrow{\text{lemma}}\dd{\mathcal{L}}=\left\langle\qty(\pdv{X_N}{\Theta})^{\!\!\top}\grad{L},\dd{\Theta}\right\rangle_F.
			$
			\smallskip
		...
		\blankfootnote{$e\coloneqq (1,\dots,1)$}
	\end{frame}
	...
	\begin{frame}{Pesi degli archi $\qquad 1/2$}{Differenziale della funzione di attivazione}
		...
		\onslide<5->{Il particolare per il MLP, $\dd{h.}(W_iX_{i-1}+b_ie^{\top})=(\Delta_i)_{\mathrm{H}}\,\dd\qty(W_iX_{i-1}+b_ie^{\top})$}
	\end{frame}
	...
	\begin{frame}
		{\hypertarget{frame:loss_derivative}{Derivata della \emph{loss} $\qquad 1/2$}}
	...
	\begin{frame}
		{Derivata della \emph{loss} $\qquad 2/2$}
		{Ricavare il gradiente dalla matrice di adiacenza}
		...
		\onslide<2->{\begin{figure}
			\centering
			\includegraphics[width=\textwidth]{Immagini/formulone_path_weights}
		\end{figure}}
...
%	\begin{frame}{Implementazione in Julia $\qquad 1/2$}{Tipo di dato \texttt{Operator} e alcuni operatori degni di nota}
%		Rappresentiamo una funzione lineare come un'istanza del tipo \texttt{Operator}, definito come uno \texttt{struct} con tre campi.
%		\begin{columns}
%			\begin{column}{0.3\textwidth}
%				\begin{figure}[htp]
%					\centering
%					\includegraphics[width=\textwidth]{Immagini/Operator_declaration}
%					\caption{\emph{Tipo di dato \texttt{Operator}}}
%				\end{figure}
%			\end{column}
%			\begin{column}{0.7\textwidth}<2->
%				\begin{figure}[htp]
%					\centering
%					\includegraphics[width=\textwidth]{Immagini/LRHIO_declarations}
%					\caption{\emph{Operatori lineari}}
%				\end{figure}
%			\end{column}
%		\end{columns}
%
%	\end{frame}
%	
%	\begin{frame}{Implementazione in Julia $\qquad 2/2$}{``Sovraccaricamento'' di alcune funzioni}
%		Introduciamo alcune funzioni che ci permetteranno di implementare la backpropagation su un MLP come sostituzione all'indietro di un sistema lineare con matrici di operatori.
%		
%		\begin{columns}
%			\begin{column}{0.5\textwidth}
%				\begin{figure}
%					\centering
%					\includegraphics[width=\textwidth]{Immagini/overloading}
%					\caption{\emph{Ridefinizione di funzioni che coinvolgono operatori}}
%				\end{figure}
%			\end{column}
%			\begin{column}{0.5\textwidth}
%				\begin{figure}
%					\centering
%					\includegraphics[width=\textwidth]{Immagini/overloading_operations}
%				\end{figure}
%			\end{column}
%		\end{columns}
%		
%	\end{frame}
	...
	\begin{frame}{Implementazione in Julia $\qquad 1/2$}{Overview del codice prodotto}
		\begin{block}{Disclaimer}
			Il codice era già fornito dagli autori dell'articolo originale. \`E stato solo leggermente rimaneggiato.
		\end{block}
		
		
		\begin{itemize}
			\item \algotext{Operator.jl}: introduzione del tipo di dato \texttt{Operator} $+$ alcuni operatori lineari $+$ definizione di funzioni/operazioni che coinvolgono operatori lineari
			\item \algotext{Notebook\_esempi.ipynb}: semplici test ed esempi d'uso
			\item \algotext{Notebook\_MLP.ipynb}: calcolo del gradiente della \emph{loss} function di un MLP tramite la risoluzione di un sistema lineare con matrici che hanno operatori come elementi {\smaller (aka backpropagation as backsubstitution)}
		\end{itemize}
		\medskip
		
%		\onslide<2->{\`E stata fatta un'indagine più approfondita di alcuni problemi e alcune criticità}% (in Notebooks separati)}
%		\begin{itemize}
%			\item<2-> \algotext{1\_Valutazione.ipynb}: L'ordine di  
%		\end{itemize}
	\end{frame}
	
	\begin{frame}
		{Implementazione in Julia $\qquad 2/2$}
		{Calcolo dei \emph{path weights}, in pratica}
			
		Abbiamo visto come arrivare a definire 
		\[
			\grad{J} = M^{\top}\qty((I - \tilde{L}) \,\backslash\, g)
		\]
		e che cosa rappresenta, per il MLP.
		\medskip
		
		\onslide<2->{L'espressione può essere valutata perché}
		\begin{enumerate}
			\item<2-> $\mathtt{g}$ è un vettore {\smaller (al netto di $\mathcal{O}$)}
			\item<3-> $\mathtt{ImL}'$ è di tipo \texttt{Bidiagonal}, e $\backslash$ implementa la sostituzione all'indietro {\smaller (unico uso di $\backslash$: $\mathcal{I}\,\backslash\,M$ con $\mathtt{M}<:$ \texttt{Matrix})}
			\item<4-> $\mathtt{M}'\,*\,(\dots)$ applica una matrice di \texttt{Operator}s a un vettore con dei \texttt{Float64}. Se $\mathtt{M}$ non è diagonale, va gestita la somma con uno zero {\smaller ($\equiv$ matrice nulla)} di dimensioni diverse!
		\end{enumerate}
	
	\end{frame}
	
	
	\begin{frame}{In conclusione\dots}{Commenti, e possibili estensioni}
		
		Backpropagation $\equiv$ Calcolo dei \emph{path weights} $\equiv$ valutazione di $\mathtt{M}'\ *\ (\mathtt{ImL}' \,\backslash\, \mathtt{g})$.
		
		\begin{itemize}
			\item L'articolo si propone di fare \emph{backpropagation as backsubstitution}. Il codice fornito mira strettamente a ciò, permettendo di fare poco altro
			\item Tentando di estendere, ci si addentra ben presto in un ginepraio
			\item Con le proprietà matematiche a disposizione, non c'è tanto di più che si può fare {\smaller (in termini di risultati ottenibili. \`E senz'altro possibile introdurre più ``regole'' anche utili)}
		\end{itemize}
		\medskip
				
		\onslide<2->{Ad \textbf{esempio} {\smaller (cfr.\algotext{Valutazione.ipynb}, \algotext{Inversione.ipynb}, \algotext{Altra\_M.ipynb})}:}
		\begin{itemize}
			\item<2-> Nella composizione di operatori mancano controlli di ammissibilità; e potrebbe ben presto diventare inefficiente
			\item<3-> Valutare $\mathtt{M}'\ *\ (\mathtt{ImL}' \,\backslash\, \mathtt{g})$ in maniera alternativa presta il fianco a molti problemi 
			\begin{itemize}
				\item<3-> Inversione esplicita di una matrice di \texttt{Operator}s
				\item<3-> Moltiplicazione di matrici di \texttt{Operator}s
				\item<3-> Somma tra matrici di dimensioni diverse, ma di cui una è nulla
			\end{itemize} 
			\item<4-> Nella pratica, avere un ordinamento topologico del DAG è \emph{fondamentale}
		\end{itemize}
...
\section*{Appendice}
	...
	\begin{frame}
		...
		\begin{dcases}
			x_n = b_n / T_{n,n} & \\
			x_k = b_k - \frac{1}{T_{k,k}}\smashoperator[r]{\sum_{j=k+1}^n} T_{k,j}x_k & k=n-1,\dots,1
		\end{dcases}
		\]
		...
		\begin{dcases}
			x_n = 1/1 = 1& \\
			x_k = -\smashoperator[lr]{\sum_{j=k+1}^n} (I-L^{\top})_{k,j}\,x_j = \smashoperator[lr]{\sum_{j=k+1}^n} (L^{\top})_{k,j}\,x_j
		\end{dcases}	
		\end{equation}
	...
	\begin{frame}{Aggiunti di operatori lineari $\qquad 4/4$}{L'aggiunta di $M_{\mathrm{H}}$}
		...
		\begin{proof}
			...
			\[
				(M_{\mathrm{H}})^{\top}=\operatorname{Vec}^{\top}\circ f^{\top}\circ\operatorname{Vec}^{-\top}=\operatorname{Vec}^{-1}\circ f\circ\operatorname{Vec}=M_{\mathrm{H}}.\qedhere
			\]
		\end{proof}
